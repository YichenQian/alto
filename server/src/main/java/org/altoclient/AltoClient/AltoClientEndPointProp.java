...
public final class AltoClientEndPointProp {
    ...
    public AltoClientEndPointProp(String hostName, String requestLine, List<String> properties,
            List<String> endpoints) {
        if (hostName == null)
            hostName = AltoClientRequestHost.EndPointPropRequestHost;
        if (requestLine == null)
            requestLine = AltoClientRequestLine.EndPointPropRequestLine;
        ...
        sEntity.setContentType(AltoClientFilteredContentTypeString.EndPointProperty);
        httpPost.setEntity(sEntity);
        httpPost.setHeader("Accept", AltoClientAcceptConst.EndPointPropAccept);
    ...
    private boolean checkResponseHeader() {
        ...
        if (httpResponse.get(0).toString().intern() != AltoClientRightString.HttpGetRightStatus.intern())
            return false;
        if (httpResponse.get(1).toString().intern() != AltoClientResponseType.EndPointPropResponseType.intern())
            return false;
        return true;
    }
            ...
            try {
                ...
                if (metaObj.has("dependent-vtags")) {
                    ...
                    for (loop = 0; loop < size; loop++) {
                        ...
                        VTag vTag = new VTag(AltoClientRightString.InvalidSourceID, AltoClientRightString.InvalidTAG);
                ...
                EPPInfo.setURI(uri);
                EPPInfo.setOperationStatus(AltoClientRightString.OperationSuccess);
            } catch (Exception e) {
                e.printStackTrace();
                EPPInfo.setOperationStatus(AltoClientRightString.OperationFailure);
            }
        } else {
            EPPInfo.setOperationStatus(AltoClientRightString.OperationFailure);
        }
    ...
    public static void mainb(String[] args) {
        ...
        altoClientEndPointProp = new AltoClientEndPointProp("alto.alcatel-lucent.com:8000", "end-props", properties,
                endpoints);
        // altoClientEndPointProp=new
        // AltoClientEndPointProp(args[0],args[1],properties,endpoints);
