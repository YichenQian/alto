...
public final class AltoClientCostMap {
    ...
    public AltoClientCostMap() {
        this(AltoClientRequestHost.NetworkMapRequestHost, AltoClientRequestLine.NetworkMapRequestLine);
    }

    public AltoClientCostMap(String hostName, String requestLine) {

        if (hostName == null)
            hostName = AltoClientRequestHost.NetworkMapRequestHost;
        if (requestLine == null)
            requestLine = AltoClientRequestLine.NetworkMapRequestLine;
        ...
        httpGet.setHeader("Accept", AltoClientAcceptConst.CostMapAccept);
    ...
    public AltoClientCostMap(String hostName, String requestLine, CostType costType, List<String> srcPids,
            List<String> dstPids, List<String> constraints) {
        if (hostName == null)
            hostName = AltoClientRequestHost.CostMapRequestHost;
        if (requestLine == null)
            requestLine = AltoClientRequestLine.CostMapRequestLine;
        this.hostName = hostName;
        this.requestLine = requestLine;
        ...
        sEntity.setContentType(AltoClientFilteredContentTypeString.CostMap);
        httpPost.setEntity(sEntity);
        httpPost.setHeader("Accept", AltoClientAcceptConst.CostMapAccept);
    ...
    private boolean checkResponseHeader() {
        ...
        if (size < 3)
            return false;
        if (httpResponse.get(0).toString().intern() != AltoClientRightString.HttpGetRightStatus.intern())
            return false;
        if (httpResponse.get(1).toString().intern() != AltoClientResponseType.CostMapResponseType.intern())
            return false;
        return true;
    }

    public void getInfo(AltoClientCostMapSwapInfo costMapInfo) {
        if (checkResponseHeader()) // begin to parse some data from response for
                                   // IRD
        {
            try {
                ...
                JSONObject costMapListObj = null;
                try {
                    costMapListObj = (JSONObject) (jsonobj.get("cost-map"));
                } catch (Exception e) {
                    costMapListObj = new JSONObject("{}");
                }
                // v tag array parse
                ...
                for (loop = 0; loop < size; loop++) {
                    ...
                    VTag vTag = new VTag(AltoClientConstString.InvalidResourceID, AltoClientConstString.InvalidTAG);
                ...
                costMapInfo.setOperationStatus(AltoClientRightString.OperationSuccess);
            } catch (Exception e) {
                e.printStackTrace();
                costMapInfo.setOperationStatus(AltoClientRightString.OperationFailure);
            }
        } else {
            costMapInfo.setOperationStatus(AltoClientRightString.OperationFailure);
        }
    ...
    public static void mainbank(String[] args) throws JSONException, UnsupportedEncodingException {
        ...
        if (bFiltered == false) {
            ...
            altoClientCostMap = new AltoClientCostMap("alto.alcatel-lucent.com:8000", "costs/filtered", postCostType,
                    srcPids, dstPids, null);
            altoClientCostMap.getInfo(costMapInfo);
            System.out.println("done");
        }
        System.out.println(costMapInfo.toString());
