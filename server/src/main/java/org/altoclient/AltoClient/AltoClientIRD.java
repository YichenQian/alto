...
public final class AltoClientIRD {
    ...
    public AltoClientIRD() {
        this(AltoClientRequestHost.IRDRequestHost, AltoClientRequestLine.IRDRequestLine);
    }

    public AltoClientIRD(String hostName, String requestLine) {

        if (hostName == null)
            hostName = AltoClientRequestHost.IRDRequestHost;
        if (requestLine == null)
            requestLine = AltoClientRequestLine.IRDRequestLine;
        ...
        httpGet.setHeader("Accept", AltoClientAcceptConst.IRDAccept);
    ...
    private boolean checkResponseHeader() {
        ...
        if (httpResponse.get(0).toString().intern() != AltoClientRightString.HttpGetRightStatus.intern())
            return false;
        if (httpResponse.get(1).toString().intern() != AltoClientResponseType.IRDResponseType.intern())
            return false;
        return true;
    }
                                   ...
                                   // IRD
        {
            try {
                ...
                while (iters.hasNext()) {
                    ...
                    CostType costTypeTmp = new CostType(key, AltoClientConstString.Invalid,
                            AltoClientConstString.Invalid, "");
                ...
                IRDInfo.setURI(uri);
                IRDInfo.setOperationStatus(AltoClientRightString.OperationSuccess);
            } catch (Exception e) {
                e.printStackTrace();
                IRDInfo.setOperationStatus(AltoClientRightString.OperationFailure);
            }
        } else {
            IRDInfo.setOperationStatus(AltoClientRightString.OperationFailure);
        }
    ...
    public static void main(String[] args) {
        ...
        // AltoClientIRD altoClientIRD=new
        // AltoClientIRD("baidu.com","directory");
        ...
        String operationStatus = IRDInfo.getOperationStatus(); // success
                                                               // failure,String
                                                               // operationStatus
