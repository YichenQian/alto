...
public final class AltoClientNetworkMap {
    ...
    public AltoClientNetworkMap(String hostName, String requestLine, List<String> pids, List<String> address_types) {
        ...
        sEntity.setContentType(AltoClientFilteredContentTypeString.NetworkMap);
        httpPost.setEntity(sEntity);
        httpPost.setHeader("Accept", AltoClientAcceptConst.NetworkMapAccept);/**/
    ...
    private boolean checkResponseHeader() {
        ...
        if (httpResponse.get(0).toString().intern() != AltoClientRightString.HttpGetRightStatus.intern())
            return false;
        if (httpResponse.get(1).toString().intern() != AltoClientResponseType.NetworkMapResponseType.intern())
            return false;
        return true;
    }
    ...
    public void getInfo(AltoClientNetworkMapSwapInfo networkMapInfo) {
                                   ...
                                   // IRD
        {
            try {
                ...
                JSONObject networkMapObj = null;
                try {
                    networkMapObj = (JSONObject) (jsonobj.get("network-map"));
                } catch (Exception e) {
                    networkMapObj = new JSONObject("{}");
                }
                ...
                while (iters.hasNext()) {
                    ...
                    JSONObject obj = null;
                    try {
                        obj = networkMapObj.getJSONObject(pidID);
                    } catch (Exception e) {
                        obj = JSONObject("{}");
                    }
                    ...
                    if (obj.has("ipv6")) {
                        ...
                        for (loop = 0; loop < size; loop++) {
                            String tmp = jsonMembers.getString(loop);
                            iPV6List.add(IPv6Network.fromString(tmp).toString());
                        }
                    }
                ...
                networkMapInfo.setOperationStatus(AltoClientRightString.OperationSuccess);
                networkMapInfo.setURI(uri);
            } catch (Exception e) {
                e.printStackTrace();
                networkMapInfo.setOperationStatus(AltoClientRightString.OperationFailure);
            }
        } else {
            networkMapInfo.setOperationStatus(AltoClientRightString.OperationFailure);
        }
    ...
    public static void main(String[] args) throws JSONException, UnsupportedEncodingException {
        ...
        if (bFiltered == false) {
            ...
            altoClientNetworkMap = new AltoClientNetworkMap("alto.alcatel-lucent.com:8000", "def-network/network/full");
            altoClientNetworkMap.getInfo(networkMapInfo);
            System.out.println("done");
        } else {
            System.out.println("begin");
            ...
            altoClientNetworkMap = new AltoClientNetworkMap("alto.alcatel-lucent.com:8000",
                    "def-network/network/filtered", pids, null);
            altoClientNetworkMap.getInfo(networkMapInfo);
            System.out.println("done");
        }
        System.out.println(networkMapInfo.toString());
