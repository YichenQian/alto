...
public final class AltoClientEndPointCost {
    ...
    public AltoClientEndPointCost(String hostName, String requestLine, CostType costType, List<String> srcEPs,
            List<String> dstEPs) {
        if (hostName == null)
            hostName = AltoClientRequestHost.EndPointCostRequestHost;
        if (requestLine == null)
            requestLine = AltoClientRequestLine.EndPointCostRequestLine;
        ...
        sEntity.setContentType(AltoClientFilteredContentTypeString.EndPointCost);
        httpPost.setEntity(sEntity);
        httpPost.setHeader("Accept", AltoClientAcceptConst.EndPointCostAccept);
    ...
    private boolean checkResponseHeader() {
        ...
        if (httpResponse.get(0).toString().intern() != AltoClientRightString.HttpGetRightStatus.intern())
            return false;
        if (httpResponse.get(1).toString().intern() != AltoClientResponseType.EndPointCostResponseType.intern())
            return false;
        return true;
    }
    ...
    public void getInfo(AltoClientEndPointCostSwapInfo endPointCostInfo) {
                                   ...
                                   // IRD
        {
            try {
                ...
                CostType costType = new CostType(AltoClientConstString.Invalid, "", "", "");
                ...
                endPointCostInfo.setOperationStatus(AltoClientRightString.OperationSuccess);
            } catch (Exception e) {
                e.printStackTrace();
                endPointCostInfo.setOperationStatus(AltoClientRightString.OperationFailure);
            }
        } else {
            endPointCostInfo.setOperationStatus(AltoClientRightString.OperationFailure);
        }
    ...
    public static void main(String[] args) throws JSONException, UnsupportedEncodingException {
        ...
        for (EndPointCost loop : endPointCostList) {
            System.out.println("{\"src\":" + "\"" + loop.getSrcEP() + "\"" + ",\"dst:\"" + loop.getDstEP()
                    + " \",\"cost\":" + loop.getCost() + "}");
        }
    ...
    public static void mainbank(String[] args) throws JSONException, UnsupportedEncodingException {
        ...
        altoClientEndPointCost = new AltoClientEndPointCost("alto.alcatel-lucent.com:8000", "costs/end", postCostType,
                srcEPs, dstEPs);
        altoClientEndPointCost.getInfo(endPointCostInfo);
        System.out.println("done");
