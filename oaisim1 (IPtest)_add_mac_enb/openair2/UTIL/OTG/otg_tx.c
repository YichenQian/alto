...
const char HEADER_STRING[] = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
const char PAYLOAD_STRING[] = "111111111111111111111111111111111111111111111111111111111111111111111111111111111111";
const char FIXED_STRING[] = "ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
...
char *packet_gen(int src, int dst, int ctime, int * pkt_size, int frame){ // when pdcp, ctime = frame cnt
  ...
  *pkt_size = 0;
  ...
  ////////////////////////////////////
  /*make change*/
  //otg_info = (otg_info_t*)malloc(sizeof(otg_info_t));
  //g_otg = (otg_t*)malloc(sizeof(otg_t));
  g_otg->application_type[0][1] = 1;
  g_otg->ip_v[0] = 1;
  g_otg->trans_proto[0] = 2;
  
  //ue
  g_otg->application_type[1][0] = 1;
  ////////////////////////////////////
...
/* if the aggregated size is less than PAYLOAD_MAX the traffic is aggregated, otherwise size=PAYLOAD_MAX */
  ...
  sprintf(definehead, "%04d%02d", frame, (ctime%10));
  
//  *definehead=*defineframe;
//  *(definehead++)=(char)*definesubframe;
//  definehead--;
  header = definehead;
  ////////////////
  
//  header =random_string(header_size_gen(src), g_otg->packet_gen_type, HEADER_ALPHABET, 2*(ctime-1));
  times++;
//  payload = random_string(size, g_otg->packet_gen_type, PAYLOAD_ALPHABET, times);

  payload = random_string(payload_size, g_otg->packet_gen_type, PAYLOAD_ALPHABET, times);
  int a, b;
  a = strlen(header);
  b = strlen(payload);
  flag=0xffff;
  ...
  header =random_string(header_size_gen_background(src),  g_otg->packet_gen_type, HEADER_ALPHABET, times);
//  payload = random_string(otg_info->size_background[src][dst],  g_otg->packet_gen_type, PAYLOAD_ALPHABET, times);
  payload = random_string(84,  g_otg->packet_gen_type, PAYLOAD_ALPHABET, times);
  flag=0xbbbb;
...
char *random_string(int size, ALPHABET_GEN mode, ALPHABET_TYPE data_type, unsigned int times) {
  //make change
  char *payload_new;
  payload_new = (char*)malloc(600);
  memset(payload_new, 49, 600);
  /////////////////////////
  char *data=NULL;
  int i=0,start=0;
  switch (mode){
  case REPEAT_STRING:
    ...
    return str_sub (FIXED_STRING, start, start + size -1, times);
    break;
  case SUBSTRACT_STRING:
    ...
    if (data_type == HEADER_ALPHABET){
      ...
      start = 0;
      //////////////////////////////
      return str_sub (HEADER_STRING, start, start + size -1, times);
    }else if (data_type == PAYLOAD_ALPHABET) {
      //make change
      //start = uniform_dist (0, abs(strlen(PAYLOAD_STRING)- size - 1));
      //return str_sub (PAYLOAD_STRING, start, start+size - 1, times);
      start = uniform_dist (0, abs(strlen(payload_new)- size - 1));
      return str_sub (payload_new, start, start+size - 1, times);
      ////////////////////////
    }else 
