...
extern struct msg_st data;
extern inline unsigned int taus(void);
extern int mac_phy_buf_size;
//make change add phy
int dl_length;
extern int semid;
extern int msgid; 
extern int shmid;
extern long int msgtype;
extern char * shmaddr;
...
s16 find_UE_RNTI(unsigned char Mod_id, unsigned char UE_id) {
  ...
  //make chage
  eNB_mac_inst[Mod_id].UE_template[UE_id].rnti = 1;
  ///////////////
...
void rx_sdu(u8 Mod_id,u32 frame,u16 rnti,u8 *sdu, u16 sdu_len) {
  ...
  //int sfid[]
  //sprintf(definehead, "/home/cic/LTE_UE/macoutput/%d_%d.txt", (count/4), count%4+6);
  sprintf(definehead, "/home/cic/LTE/mac_output/0_6.txt");
  FILE *fp=fopen(definehead,"r");
  rlength = fread(sdu,1,1024,fp);
  fclose(fp);
  count++;
//  fp=fopen("/home/administrator/test.txt","w");
//  fwrite(sdu,349,1,fp);
//  fclose(fp);*/
  /////////////////////////
  ...
  //make change
//  fp=fopen("/home/administrator/test_rx_payload.txt","w");
//  fwrite(payload_ptr,1611,1,fp);
//  fclose(fp);
  ///////////////////////////
...
void tm5_pre_processor (unsigned char Mod_id,
  ...
  for (UE_id=0;UE_id<granted_UEs;UE_id++) {
	...
	break;
      ...
      if (next_ue == 255) //problem?
	break;

      // This is an allocated UE_id
      rnti = find_UE_RNTI(Mod_id,next_ue);
      eNB_UE_stats = mac_xface->get_eNB_UE_stats(Mod_id,rnti); //mark
...
//rnti_temp
  ...
  if(k3 == 1)
	...
	if(((ue[1][3] == ue[0][0]) || (ue[1][3] == ue[1][0]))&&
      ...
      rballoc_sub[ue[1][3]][3] = 1;//ue二位数组,rb二位数组？
    }
...
\
	...
	*/
	...
	//eNB_mac_inst[0].DLSCH_pdu[0][0].payload[0][offset+sdu_lengths[0]+j] = (char)(taus()&0xff);
        ...
        char *name;
        name = malloc(100);
        sprintf(name, "/home/cic/LTE_UE/ue_macoutput/%04d%s%04d.txt", frame, "_", subframe);
        FILE *fp=fopen(name,"w");
        fwrite(&eNB_mac_inst[Mod_id].DLSCH_pdu[(unsigned char)next_ue][0].payload,TBS,1,fp);
        fclose(fp);
        */
        ///////////////////
        /*
        if((shmid = creatshm("/home/cic/enb_mac_phy/", 57, SHM_SIZE)) == -1) //create or get shared memory
            return -1;
        ...
        */
        memcpy(&dl_length,shmaddr,sizeof(int));
        while (dl_length >= MAX_LENGTH)
        {
            usleep(10);
            memcpy(&dl_length,shmaddr,sizeof(int));
        }
        if (dl_length < MAX_LENGTH)
        {
            /*
            ...
            */
            //pthread_mutex_lock(&t_mutex); // 给互斥体变量加锁 
            printf("write : ");
            data.msg_type = 1;
            data.length = TBS;
            memcpy(data.text,&eNB_mac_inst[Mod_id].DLSCH_pdu[(unsigned char)next_ue][0].payload[0],TBS);
            /*int error = msgsnd(msgid, (void*)&data, sizeof(int)+sizeof(long int)+TBS, IPC_NOWAIT);
            ...
            if(msgsnd(msgid, (void*)&data, sizeof(int)+sizeof(long int)+TBS, 0) == -1)
            {  
            ...
            //wait_sem(semid, 0);//wait for sem
            //sem_p(semid, 0);  //get sem
            /**************write shared memory***************************************************/
            //memcpy(&shmaddr[mac_phy_buf_size+4],&eNB_mac_inst[Mod_id].DLSCH_pdu[(unsigned char)next_ue][0].payload[0],TBS);
            //mac_phy_buf_size = mac_phy_buf_size + TBS;
            //memcpy(shmaddr,&length,sizeof(int));
            /****************************************************************************/
            sem_v(semid, 0); //release sem
            ////////////////////////
            //pthread_mutex_unlock(&t_mutex); // 给互斥体变量解除锁
            //fp=fopen("/home/cic/enb_mac_phy/write.txt","w");
            //fwrite(shmaddr,TBS,1,fp);
            //fclose(fp);
            /*
            sprintf(name, "/home/cic/enb_mac_phy/write_size %d%s%d.txt", frame, "_", subframe);
            fp=fopen(name,"w");
            int size = 0;
            memcpy(&size,shmaddr,sizeof(int));
            size = size+1;
            ...
            */
            //TODO mark
            /*
            sprintf(name, "/home/cic/LTE_UE/enb_mac_phy/write %d%s%d.txt", frame, "_", subframe);
            fp=fopen(name,"w");
            //fwrite(&shmaddr[mac_phy_buf_size+4-TBS],TBS,1,fp);
            fwrite(data.text,TBS,1,fp);
            fclose(fp);
             * */
            ////////////////////////
        }
