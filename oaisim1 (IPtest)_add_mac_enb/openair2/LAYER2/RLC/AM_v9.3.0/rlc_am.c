...
/* ONLY ONE TB PER TTI
            ...
            if ((rlcP->nb_bytes_requested_by_mac > 2) && (rlcP->vt_s != rlcP->vt_ms)) {
              //it's the problem
              //make change
//              FILE *fp=fopen("/home/administrator/segment_ag","a+");
//              fprintf(fp, "frame=%d  nb_bytes=%d  vt_s=%d vt_ms=%d \n", frame, rlcP->nb_bytes_requested_by_mac, rlcP->vt_s, rlcP->vt_ms);
//              fclose(fp);
              ///////////////////
	      ...
	      list_add_list (&rlcP->segmentation_pdu_list, &rlcP->pdus_to_mac_layer);
              ...
              //make change
//               FILE *fp=fopen("/home/administrator/mac_to_pdu.txt","w");
//                fwrite(rlcP->pdus_to_mac_layer.head->data,120,1,fp);
//                fclose(fp);
                ////////////////////
...
//-----------------------------------------------------------------------------
  ...
  //make change
  /*
  FILE *fp=fopen("/home/cic/rlc_output.txt","w");
  fwrite(l_rlc->pdus_to_mac_layer.head->data,1598,1,fp);
  fclose(fp);
 */
  ////////////////////
  ...
  //make change
 // fp=fopen("/home/cic/mac_input.txt","w");
  //fwrite(data_req.data.head->data,120,1,fp);
  //fclose(fp);
  ////////////////////
    ...
    if (data_indP.data.nb_elements > 0) {
        ...
        while (tb != NULL) {
            if ((((struct mac_tb_ind *) (tb->data))->data_ptr[0] & RLC_DC_MASK) == RLC_DC_DATA_PDU ) {
                ...
                //make change
 //               FILE *fp=fopen("/home/cic/LTE_UE/rlc_am_get_data_pdu_infos.txt","w");
 //               fwrite((rlc_am_pdu_sn_10_t*) ((struct mac_tb_ind *) (tb->data))->data_ptr,1,(s16_t) ((struct mac_tb_ind *) (tb->data))->size,fp);
//                fclose(fp);
                ////////////////////   
...
// rlc->input_sdus = (rlc_am_entity_t*) malloc (10*sizeof(rlc_am_entity_t));
...
//    char *datasdu;
//    datasdu = malloc(100);
//    sprintf(datasdu, "NB SDU %d current_sdu_index=%d next_sdu_index=%d\n", rlc->current_sdu_index, rlc->next_sdu_index);
//    FILE *fp=fopen("/home/administrator/rlc_current_sdu","a+");
//    fwrite(datasdu,50,1,fp);
//    fprintf(fp, "frame=%d  current_sdu_index=%d  next_sdu_index=%d \n", frame, rlc->current_sdu_index, rlc->next_sdu_index);
//    fclose(fp);
    ///////////////////
